...
The BIC approximation to the Bayes Factor shows that the the data are more likely under the model of main effects with no interaction than the model including the interaction term for both by-subjects and by-items analyses (by-subjects: $BIC(H_1)$ = `r round(BIC(main_subj), 3)`, $BIC(H_2)$ = `r round(BIC(max_subj), 3)`, $BF_{12} \approx$ `r round(bf_against_int_subj, 3)`; by-items: $BIC(H_1)$ = `r round(BIC(main_items), 3)`, $BIC(H_2)$ = `r round(BIC(max_items), 3)`, $BF_{12} \approx$ `r round(bf_against_int_items, 3)`). This suggests that the discourse condition plays no role in modulating visual competition during the critical noun region + 400ms.
