...
The BIC approximation to the Bayes Factor shows that the the data are more likely under the null than the alternative hypothesis for both by-subjects and by-items analyses (by-subjects: $BIC(H_0)$ = `r round(BIC(cond_dist_subj_min), 3)` $BIC(H_1)$ = `r round(BIC(cond_dist_subj_max), 3)`, $BF_{01} \approx$ `r round(cond_dist_bf_subj, 3)`; by-items: $BIC(H_0)$ = `r round(BIC(cond_dist_items_min), 3)` $BIC(H_1)$ = `r round(BIC(cond_dist_items_max), 3)`, $BF_{01} \approx$ `r round(cond_dist_bf_items, 3)`). This suggests that fixations on the distractor are not influenced by the discourse condition.
...
The BIC approximation to the Bayes Factor shows that the the data are more likely under the null than the alternative hypothesis for both by-subjects and by-items analyses (by-subjects: $BIC(H_0)$ = `r round(BIC(ia_apart_subj_min), 3)` $BIC(H_1)$ = `r round(BIC(ia_apart_subj_max), 3)`, $BF_{01} \approx$ `r round(ia_apart_bf_subj, 3)`; by-items: $BIC(H_0)$ = `r round(BIC(ia_apart_items_min), 3)` $BIC(H_1)$ = `r round(BIC(ia_apart_items_max), 3)`, $BF_{01} \approx$ `r round(ia_apart_bf_items, 3)`). This suggests that there is no difference in the fixations on the competitor and distractor in the Apart condition.
